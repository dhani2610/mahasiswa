...
class UserController extends Controller
{
        ...
        $cekMail = User::where('username',$val)->first();
            ...
            if($cekMail != null){
                ...
                if (Auth::attempt($credentials)) {
                    ...
                    // dd($cekMail->getRoleNames(),$request->all());
                    if ($from == 'admin'){
                        // dd('masuk',$cekMail->getRoleNames()[0] == 'SuperAdmin',$cekMail->getRoleNames());
                        if ($cekMail->getRoleNames()[0] == 'SuperAdmin'){
                            return redirect()->route('dashboard.index');
                        }else{
                            return redirect()->back()->with(['failed' => 'No Credentials']);
                        }
                    }elseif ($from == 'siswa'){
                        if ($cekMail->getRoleNames()[0] == 'Siswa'){
                            return redirect()->route('dashboard.index');
                        }else{
                            return redirect()->back()->with(['failed' => 'No Credentials']);
                        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $user->username = $validateData['username'];
        $user->email = $validateData['email'];
        $user->divisi_id = $validateData['divisi_id'];
